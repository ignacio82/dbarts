...
namespace dbarts {
  ...
  void updateOrdinalVariablesAvailable(const BARTFit& fit, Node* node, int32_t variableIndex, int32_t leftIndex, int32_t rightIndex)
  {
    ...
    if (!node->isBottom()) {
      ...
      if (node->getRule().variableIndex == variableIndex) {
        leftChildRightIndex = node->getRule().splitIndex - 1;
        rightChildLeftIndex = node->getRule().splitIndex + 1;
      }
      ...
      if (node->getRule().variableIndex == variableIndex) {
        ...
        for (size_t i = 0; i < numCategories; ++i) {
          if (catGoesRight[i] == true) {
            if ((node->getRule().categoryDirections & categoryMask) != 0) {
  ...
  void setCategoryReachability(const BARTFit& fit, const Node& node, int32_t variableIndex, bool* categoriesCanReachNode)
  {
    ...
    while (!curr->isTop()) {
      parent = curr->parent;
      if (parent->getRule().variableIndex == variableIndex) {
        if (curr == parent->getLeftChild()) {
          for (uint32_t i = 0; i < numCategories; ++i) { // parent woulda taken this right, but we're left child
            if (parent->getRule().categoryGoesRight(i)) categoriesCanReachNode[i] = false;
          }
        } else {
          for (uint32_t i = 0; i < numCategories; ++i) {
            if (parent->getRule().categoryGoesRight(i) == false) categoriesCanReachNode[i] = false;
    ...
    while (!curr->isTop() && !(leftFound && rightFound)) {
      ...
      if (curr->getRule().variableIndex == variableIndex) {
        if (isRightChild && !leftFound) {
          leftFound = true;
          *leftIndex = curr->getRule().splitIndex + 1;
        }
        if (!isRightChild && !rightFound) {
          rightFound = true;
          *rightIndex = curr->getRule().splitIndex - 1;
        }
      }
    }
  }
